function [fields, obj] = displacementField(obj, angleChoice, freqChoice, varargin)
...
%     disp(['Frequency chosen: '  ,   num2str(freq_vec/1e6), ' MHz'])
    ...
    phaseVel = sqrt(obj.medium(1).cMat(1,1) / obj.medium(1).density);
...
%     num_of_interfaces = numLayers - 1;
    ...
    for layIdx = 1:numLayers
        [ matProp(layIdx).alpha, matProp(layIdx).cMat, matProp(layIdx).pVec ] = ...
            ...
            obj.medium(layIdx).cMat, cp, obj.medium(layIdx).density );
    end
    ...
    for tdx = time
        ...
        for layerIdx = 1:numLayers
            ...
            fCorr = 1;
            if matProp(layerIdx).cMat(5,5) < 5
            ...
            switch layerIdx
                case 1 
                    ...
                    sigmaZZ =  Psy .* stressNorm .* ( ...
                        e1 * (matProp(layerIdx).cMat(1,3) + (matProp(layerIdx).cMat(3,3)) * matProp(layerIdx).alpha(1) * matProp(layerIdx).pVec(1)) + ...
                        e3 * (matProp(layerIdx).cMat(1,3) + (matProp(layerIdx).cMat(3,3)) * matProp(layerIdx).alpha(3) * matProp(layerIdx).pVec(3)) + ...
                        e4 * (matProp(layerIdx).cMat(1,3) + (matProp(layerIdx).cMat(3,3)) * matProp(layerIdx).alpha(4) * matProp(layerIdx).pVec(4)) ) ;               
                    
                    % shear stress
                    st2 = matProp(layerIdx).cMat(5, 5) * stressNorm; % a coefficient
                    sigmaXZ =  Psy .* fCorr .* (...
                ...
                case numLayers
                    ...
                    uz = Psy .* (...
                     ...
                     % normal stress       
                    sigmaZZ =  Psy .* stressNorm .* ( ...
                        e2 * (matProp(layerIdx).cMat(1,3) + (matProp(layerIdx).cMat(3,3)) * matProp(layerIdx).alpha(2) * matProp(layerIdx).pVec(2)) + ...
                        e4 * (matProp(layerIdx).cMat(1,3) + (matProp(layerIdx).cMat(3,3)) * matProp(layerIdx).alpha(4) * matProp(layerIdx).pVec(4)) ) ;               
